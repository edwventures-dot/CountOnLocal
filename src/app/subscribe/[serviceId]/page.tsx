 ...
 *
 * Called /subscribe rather than /checkout, because there is no checkout.
 * Nothing is charged here and no card is collected; what happens is that
 * two neighbours agree a standing arrangement and the app starts keeping
 * the schedule for it.
 *
...
export default async function SubscribePage({ params }: Params) {
  ...
  if (!auth.ok) {
    redirect(`/signin?next=${encodeURIComponent(`/subscribe/${serviceId}`)}`)
  }
  ...
  return (
    <Shell nav={<SignOutButton />} narrow>
      ...
      <Card>
        <Subscribe serviceId={serviceId} serviceCatalogHint={catalogCode} />
      </Card>
    </Shell>
  )
}
