...
import { previewCheckout, previewSchema } from '@/server/subscribeService'
...
export async function POST(req: Request): Promise<Response> {
  ...
  return apiOk({
    ...
    // What the provider charges, as information. There is no quote here
    // because there is no bill: the old response carried a subtotal, a
    // platform fee, a total and the effective fee rate, and every one of
    // those was a number about a charge this product no longer makes.
    price: { cents: p.priceCents, unit: p.priceUnit },
    schedule: { cycleWeeks: p.billingCycleWeeks },
